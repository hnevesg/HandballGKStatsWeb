...
public class MenuControllerFixedShoots : MonoBehaviour
{
    public ToggleInfo[] shootPositions;
    public ToggleInfoPlaces[] shootPositionPlaces;
    ...
    [SerializeField] private Toggle ToggleEscuadraDerecha;
    [SerializeField] private Toggle ToggleBajoDerecha;
    [SerializeField] private Toggle ToggleCentroDerecha;
    [SerializeField] private Toggle ToggleCentroDerechaBajo;
    [SerializeField] private Toggle ToggleCentro;
    [SerializeField] private Toggle ToggleCentroBajo;
    [SerializeField] private Toggle ToggleCentroIzquierda;
    [SerializeField] private Toggle ToggleCentroIzquierdaBajo;
    [SerializeField] private Toggle ToggleEscuadraIzquierda;
    [SerializeField] private Toggle ToggleBajoIzquierda;
    ...
    void Start()
    {
        ...
        foreach (var positionDest in shootPositionPlaces)
        {
            positionDest.toggle.onValueChanged.AddListener((value) => OnToggleValueChangedForPositionDest(positionDest.type, value));
        }
    ...
    public void StartBtn()
    {
            ...
            || (Constants_Handball.handsType == HandsType.Right && OVRInput.IsControllerConnected(OVRInput.Controller.RTouch)))
        {
            ...
            if (!DifficultyChanged)
            {
                ...
                Constants_Handball.frecuenciaLanzamientoFija = float.Parse(valueTextFrequency.text);
                Constants_Handball.velocidadLanzamientoFija = float.Parse(valueTextSpeed.text);
            }
            if (isAtLeastOnePlaceSelected() & isAtLeastOnePositionSelected())
            {
                canvasController.ComenzarCuentaAtras(true);
            }
    ...
    private void OnDropdownValueChanged(int index)
    {
        ...
        Constants_Handball.Mode = Mode.FixedShoots;
    }
    ...
    private void OnToggleValueChangedForPositionDest(ShootPositionPlaces type, bool value)
    {
        switch (type)
        {
            case ShootPositionPlaces.EscuadraDerecha:
                Constants_Handball.EscuadraDerechaCanonActivated = value;
                break;
            case ShootPositionPlaces.BajoDerecha:
                Constants_Handball.BajoDerechaCanonActivated = value;
                break;
            case ShootPositionPlaces.CentroDerecha:
                Constants_Handball.CentroDerechaCanonActivated = value;
                break;
            case ShootPositionPlaces.CentroDerechaBajo:
                Constants_Handball.CentroDerechaBajoCanonActivated = value;
                break;
            case ShootPositionPlaces.Centro:
                Constants_Handball.CentroCanonActivated = value;
                break;
            case ShootPositionPlaces.CentroBajo:
                Constants_Handball.CentroBajoCanonActivated = value;
                break;
            case ShootPositionPlaces.CentroIzquierda:
                Constants_Handball.CentroIzquierdaCanonActivated = value;
                break;
            case ShootPositionPlaces.CentroIzquierdaBajo:
                Constants_Handball.CentroIzquierdaBajoCanonActivated = value;
                break;
            case ShootPositionPlaces.EscuadraIzquierda:
                Constants_Handball.EscuadraIzquierdaCanonActivated = value;
                break;
            case ShootPositionPlaces.BajoIzquierda:
                Constants_Handball.BajoIzquierdaCanonActivated = value;
                break;
        }
    }

    public bool isAtLeastOnePlaceSelected()
    {
        if (Constants_Handball.EscuadraDerechaCanonActivated || Constants_Handball.BajoDerechaCanonActivated || Constants_Handball.CentroDerechaBajoCanonActivated || Constants_Handball.CentroDerechaCanonActivated || Constants_Handball.CentroCanonActivated ||
            Constants_Handball.CentroBajoCanonActivated || Constants_Handball.CentroIzquierdaCanonActivated || Constants_Handball.CentroIzquierdaBajoCanonActivated || Constants_Handball.EscuadraIzquierdaCanonActivated || Constants_Handball.BajoIzquierdaCanonActivated)
        {
            return true;
        }
        else
    ...
    private void updateNewConfigurationInComponentes()
    {
        ...
        if (ToggleEscuadraDerecha.isOn)
        {
            Constants_Handball.EscuadraDerechaCanonActivated = true;
        }
        else
        {
            Constants_Handball.EscuadraDerechaCanonActivated = false;
        }

        if (ToggleBajoDerecha.isOn)
        {
            Constants_Handball.BajoDerechaCanonActivated = true;
        }
        else
        {
            Constants_Handball.BajoDerechaCanonActivated = false;
        }

        if (ToggleCentroDerecha.isOn)
        {
            Constants_Handball.CentroDerechaCanonActivated = true;
        }
        else
        {
            Constants_Handball.CentroDerechaCanonActivated = false;
        }

        if (ToggleCentroDerechaBajo.isOn)
        {
            Constants_Handball.CentroDerechaBajoCanonActivated = true;
        }
        else
        {
            Constants_Handball.CentroDerechaBajoCanonActivated = false;
        }

        if (ToggleCentro.isOn)
        {
            Constants_Handball.CentroCanonActivated = true;
        }
        else
        {
            Constants_Handball.CentroCanonActivated = false;
        }

        if (ToggleCentroBajo.isOn)
        {
            Constants_Handball.CentroBajoCanonActivated = true;
        }
        else
        {
            Constants_Handball.CentroBajoCanonActivated = false;
        }

        if (ToggleCentroIzquierda.isOn)
        {
            Constants_Handball.CentroIzquierdaCanonActivated = true;
        }
        else
        {
            Constants_Handball.CentroIzquierdaCanonActivated = false;
        }

        if (ToggleCentroIzquierdaBajo.isOn)
        {
            Constants_Handball.CentroIzquierdaBajoCanonActivated = true;
        }
        else
        {
            Constants_Handball.CentroIzquierdaBajoCanonActivated = false;
        }

        if (ToggleEscuadraIzquierda.isOn)
        {
            Constants_Handball.EscuadraIzquierdaCanonActivated = true;
        }
        else
        {
            Constants_Handball.EscuadraIzquierdaCanonActivated = false;
        }

        if (ToggleBajoIzquierda.isOn)
        {
            Constants_Handball.BajoIzquierdaCanonActivated = true;
        }
        else
        {
            Constants_Handball.BajoIzquierdaCanonActivated = false;
        }
    }
}

[System.Serializable]
public class ToggleInfoPlaces
{
    public ShootPositionPlaces type;
    public Toggle toggle;
    public ToggleInfoPlaces(ShootPositionPlaces type, Toggle toggle)
    {
        this.type = type;
        this.toggle = toggle;
    }
...
public enum ShootPositionPlaces
{
    EscuadraDerecha,
    BajoDerecha,
    CentroDerecha,
